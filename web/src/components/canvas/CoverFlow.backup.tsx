...
export default function CoverFlow({ onAppChange, onNavigate }: { onAppChange?: (app: any) => void, onNavigate?: (slug: string) => void }) {
   ...
   const mousePositionRef = useRef({ x: 0.5, isInside: false });
   ...
   useEffect(() => {
      ...
      const handleMouseMove = (e: MouseEvent) => {
         ...
         if (isInside) {
            // Normalize mouse X position to 0-1 range within the container
            const normalizedX = (e.clientX - rect.left) / rect.width;
            mousePositionRef.current = { x: normalizedX, isInside: true };
         } else {
            mousePositionRef.current.isInside = false;
         }
      };

      const autoScroll = () => {
         const now = Date.now();
         const { x, isInside } = mousePositionRef.current;

         // Only auto-scroll if mouse is inside the container and enough time has passed
         if (isInside && now - lastUpdate > 300) {
            // Dead zone in the center (0.35 - 0.65)
            if (x < 0.35) {
               // Mouse on left side -> scroll left (previous)
               setActiveIndex(prev => Math.max(prev - 1, 0));
               lastUpdate = now;
            } else if (x > 0.65) {
               // Mouse on right side -> scroll right (next)
               setActiveIndex(prev => Math.min(prev + 1, COVER_ITEMS.length - 1));
               lastUpdate = now;
            }
         }
   ...
   return (
      <div
         ...
         style={{ width: '100%', height: '100%', background: 'black', touchAction: 'none', overflow: 'hidden' }}
