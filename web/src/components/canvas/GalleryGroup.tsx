...
import { useScroll, Billboard, Text, Image } from '@react-three/drei';
...
export default function GalleryGroup({ position = [0, 0, 0], ...props }) {
   ...
   useFrame((state, delta) => {
      if (ref.current) {
         // Scroll Logic:
         // 1. Rotation: Rotate based on total scroll offset
         ref.current.rotation.y = -scroll.offset * (Math.PI * 4);

         // 2. Vertical Movement: Move up as user scrolls down
         // Start very low (y=-25), animate up to center (y=0) as scroll reaches end
         const startY = (position[1] !== undefined ? position[1] : -25);
         ref.current.position.y = startY + (scroll.offset * 30);
      }

      // Simple camera movement - only apply if this section is potentially active?
      // Or apply subtly globally. 
      // Note: Main page might have other camera logic (Experience.tsx). 
      // We should check if we conflict with Experience.tsx rigging.
      // For now, let's omit camera position override to avoid breaking main page nav.
      // camera.lookAt(0, 0, 0); 
   });
