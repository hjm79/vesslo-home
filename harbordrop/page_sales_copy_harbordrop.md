# DropHarbor 판매 카피 (Vesslo 페이지 통일 버전)

이 문서는 `https://vesslo.top/keyharbor`, `https://vesslo.top/splitswipe` 페이지 구조와 톤에 맞춰 작성한 DropHarbor 판매 문구입니다.

## 1) 페이지 아이덴티티

- 페이지 경로: `/dropharbor`
- 표시 이름: `DropHarbor` (앱 표시명 HarborDrop과 동일 제품)
- 핵심 카테고리: `Download Manager for macOS`
...
## 4) 제품 페이지 섹션 카피 (KeyHarbor/SplitSwipe 구조 동일)

### A. Hero

#### EN
- `h1`: `DropHarbor`
- `tagline`: `Smart Download Management for macOS`
...
- `primary_cta`: `Purchase`

#### KO
- `h1`: `DropHarbor`
- `tagline`: `macOS용 스마트 다운로드 매니저`
- `badge`: `다운로드 매니저`
- `highlights`:
1. `멀티 세그먼트 가속 다운로드`
![[/Users/hjm/Documents/git/app.hjm79.top/harbordrop/harbordrop-segments.jpeg]]
...
#### EN
- `title`: `In Action`
- `body`: `DropHarbor keeps your downloads fast and predictable with segmented transfers, stable resume logic, and clear status tracking for active, completed, and failed items.`
![[/Users/hjm/Documents/git/app.hjm79.top/harbordrop/harbordrop-inaction.jpeg]]

#### KO
- `title`: `실제 동작`
- `body`: `DropHarbor는 세그먼트 분할 다운로드, 안정적인 이어받기, 명확한 상태 추적으로 현재 다운로드/완료/실패를 일관되게 관리합니다.`
![[/Users/hjm/Documents/git/app.hjm79.top/harbordrop/harbordrop-inaction.jpeg]]

### C. Key Features (3-card)
...
- `body`: `Move from basic downloading to an organized, automation-ready workflow on macOS.`
- `button`: `Purchase`

#### KO
- `title`: `지금 바로 시작하세요`
- `body`: `단순 다운로드를 넘어, 정리되고 자동화된 macOS 다운로드 워크플로우를 시작하세요.`
- `button`: `구매하기`
...
```json
{
  "dropharbor_page": {
    ...
    "in_action_body": "DropHarbor keeps your downloads fast and predictable with segmented transfers, stable resume logic, and clear status tracking for active, completed, and failed items.",
    "features": [
      {
        "icon": "⚡",
        "title": "Speed by Segments",
    ...
    "cta_body": "Move from basic downloading to an organized, automation-ready workflow on macOS.",
    "cta_button": "Purchase"
  }
}
```
...
```json
{
  "dropharbor_page": {
    ...
    "in_action_body": "DropHarbor는 세그먼트 분할 다운로드, 안정적인 이어받기, 명확한 상태 추적으로 현재 다운로드/완료/실패를 일관되게 관리합니다.",
    "features": [
      {
        "icon": "⚡",
        "title": "세그먼트 가속",
    ...
    "cta_body": "단순 다운로드를 넘어, 정리되고 자동화된 macOS 다운로드 워크플로우를 시작하세요.",
    "cta_button": "구매하기"
  }
}
```
...
- 과장 금지: `모든 사이트 100% 지원` 같은 문구는 사용하지 않기
- 권장 스크린샷/영상 순서:
1. 메인 다운로드 목록(현재/완료/실패)
2. 카테고리 규칙 및 후처리 작업 설정
3. HLS 다운로드 진행 화면(속도/세그먼트 상태)
- 기존 제품 페이지와 동일하게 마지막 CTA 버튼 라벨은 `Purchase/구매하기` 유지
